...
gsigs = 'https://github.com/sherkitty/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('sherkitty'):
        # Remove the potentially stale sherkitty dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'sherkitty', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'sherkitty'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/sherkitty/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'sherkitty='+args.commit, '--url', 'sherkitty='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/sherkitty-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/sherkitty/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/sherkitty/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/sherkitty/sherkitty', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/sherkitty', exist_ok=True)
    os.chdir('builder/inputs/sherkitty')
    if args.pull:
