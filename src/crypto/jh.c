...
typedef struct {
	...
	DATA_ALIGN16(uint64 x[8][2]);     /*the 1038-bit state, ( x[i][0] || x[i][1] ) is the ith row of the state in the pseudocode*/
...
static void F8(hashState *state)
{
      ...
      /*xor the 512-bit message with the fist half of the 1038-bit hash state*/
      for (i = 0; i < 8; i++)  state->x[i >> 1][i & 1] ^= ((uint64*)state->buffer)[i];

      /*the bijective function E8 */
      E8(state);

      /*xor the 512-bit message with the second half of the 1038-bit hash state*/
