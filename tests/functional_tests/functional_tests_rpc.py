...
# a main offline sherkittyd, does most of the tests
# a restricted RPC sherkittyd setup with RPC payment
# two local online sherkittyds connected to each other
N_SHERKITTYDS = 4

# 4 wallets connected to the main offline sherkittyd
# 1 wallet connected to the first local online sherkittyd
# 1 offline wallet
N_WALLETS = 6
...
sherkittyd_base = [builddir + "/bin/sherkittyd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "sherkittyd_p2p_port", "--rpc-bind-port", "sherkittyd_rpc_port", "--zmq-rpc-bind-port", "sherkittyd_zmq_port", "--zmq-pub", "sherkittyd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "sherkittyd_data_dir", "--log-level", "1"]
sherkittyd_extra = [
  ["--offline"],
